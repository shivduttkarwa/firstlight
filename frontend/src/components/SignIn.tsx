...
/** Phone number, then the six digit code. `page` gives it the big headings of the sign-in page. */
export function PhoneSignIn({ page = false }: { page?: boolean }) {
      ...
      {stage === "phone" ? (
        ...
        >
          {page ? (
            ...
            <p className="sm muted">We text you a six digit code. No password to remember.</p>
          )}

          <form onSubmit={requestCode} className={page ? "mt-3" : "mt-2"}>
        ...
        <motion.div
          ...
          transition={{ duration: 0.25 }}
        >
          {page && <h1 className="display mt-3">Enter the code.</h1>}
          <p className={page ? "lede mt-1" : "sm muted"}>
            Sent to {phone}.{" "}
