# Ships main to the Firstlight server. .\deploy.ps1, or .\deploy.ps1 -Ref <commit> to go back to an earlier one.
param(
  [string]$Ref,
  [string]$Server = "firstlight"
)
...
ssh $Server "sudo bash /srv/firstlight/app/deploy/deploy.sh $Ref"
